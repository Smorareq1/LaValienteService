...
El NIT **no** vive aquí sino en §3.2: es un dato del cliente, y pedirlo antes de
saber quién es deja un campo que no se sabe a quién pertenece.
...
| NIT | Campo texto (`nit`) | ✖ | **Solo aparece con un cliente elegido.** Sale de él: el suyo si lo tiene registrado, `CF` si no. Cambiar de cliente trae el del nuevo y soltarlo lo borra. Botón rápido "CF". Editable: manda lo último que se toca. Con la sección cerrada, el resumen lee "Nombre · NIT X" |
...
| Lavado (`wash_tub`, por tina) | Stepper por opción: Grande, Mediano, Pequeño | Combinables (1 G + 1 P). Cada opción muestra su precio; línea muestra `N × precio`. En el catálogo se llama **«Lavado»** a secas: el tamaño lo dice la opción y la unidad sigue siendo la tina |
...
- Botones: **Guardar** (`POST /orders`), **Escanear boleta** (entrada al Plan 0003),
  **Limpiar** — con confirmación, porque vive donde cae el pulgar al terminar de contar
  prendas y lo que borra no está guardado en ninguna parte.
- Tras guardar: pantalla de confirmación con el **No. diario asignado**, total, saldo, y
  acciones "Compartir comprobante" · "Nuevo pedido" / **"Listo"** (la primaria: tomar otra
  boleta seguida es la excepción). El comprobante lleva cliente, teléfono, NIT, las prendas
  contadas y la hora de recepción (Plan 0006 §17.1).
...
| 2026-08-09 | **Tres correcciones del cliente sobre la pantalla en uso** (bitácora completa en el [plan 0006](../0006-ui-app-fase1/PLAN.md)). **(a)** El **NIT** del §3.1 deja de prellenarse "solo si está vacío": sale del cliente siempre —el suyo, o `CF` si no tiene registrado— y cambiar de cliente trae el del nuevo, porque un NIT heredado del cliente anterior es una factura a nombre de quien no vino. Sigue siendo editable. **(b)** **«Limpiar boleta» pregunta antes**: el botón está al pie de la lista, justo donde cae el pulgar al terminar de contar prendas, y borra una boleta entera que no está guardada en ninguna parte. **(c)** En la confirmación del §3.8, **«Listo» es la acción primaria** —magenta y el sitio del pulgar— y «Nuevo pedido» queda sin fondo: tomar otra boleta seguida es la excepción, no lo que pasa después de cada pedido. Y el comprobante que se comparte deja de ser solo cifras: lleva **cliente, teléfono, NIT, las prendas contadas y la hora de recepción**, que es lo que se coteja al recoger la ropa. Para eso `SavedOrder` carga esos datos —los pone el controlador de la captura y no el repositorio, que solo tiene ids: los nombres de las prendas viven en el catálogo. |
| 2026-08-09 | **El NIT se muda a la sección del cliente** (§3.2) y desaparece del encabezado (§3.1). Lo pidió el cliente y la razón es la que ya estaba escrita a medias: el NIT sale del cliente, así que preguntarlo tres secciones antes de saber quién es dejaba un campo huérfano en la parte de la boleta que casi nadie abre. Ahora **solo existe con un cliente elegido**, aparece pegado a la tarjeta del cliente, y soltar al cliente se lo lleva —en la pantalla y en el estado (`setCustomer(null)` limpia `nit`)—, porque un NIT que sobrevive a su dueño es el que se cuela en la boleta del siguiente. El resumen de la sección cerrada pasa a leer «Nombre · NIT X»: cliente y NIT son lo mismo que hay que revisar de un vistazo. |
