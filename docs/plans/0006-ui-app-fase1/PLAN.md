...
| **UI 10 — Admin y pulido** | Catálogo/precios/prendas admin, dispositivos, escaneo IA (0003), comprobante por *share sheet* (§17.1), estados vacíos/errores finales | PR 12, 0003 |
...
4. **Usuarios y roles desde la app** (§13): ¿se exponen endpoints CRUD de `identity` en la
   ...
   fase 2?
   → **Decidido: sin pantalla en la fase 1.** La premisa de la pregunta ya no aplica: los
   endpoints **existen** y están protegidos —`POST /auth/register` con
   `authorization.users.manage`, más `GET /authorization/users`,
   `PUT /authorization/users/{id}/roles` y `PUT /authorization/users/{id}/permissions`—,
   así que lo único que falta es la pantalla. El primer administrador seguirá saliendo de
   `create_superuser.py` por el huevo y la gallina (alguien tiene que tener el permiso
   antes de que nadie pueda registrar), y el resto se administra por API hasta la fase 2.
   Revertirlo es barato: es UI sobre una API que ya está.
