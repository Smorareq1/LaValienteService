...
| Acción principal (una, la del estado) | `recibido` → **Pasar a proceso** · `en proceso` → **Marcar listo** · `listo` → **Cobrar** (§5.6) + **Entregar pedido** (§5.5) |
| Acciones secundarias | **Agregar servicios**, **Editar** (reabre la pantalla 0002 precargada), **Anular** (§5.7), **Regresar a…** (deshace un paso) |

**Un paso a la vez, y en orden** (revisión del 2026-08-09). La barra de abajo ofrece
únicamente el paso siguiente de la cadena: mientras la ropa está en la lavadora no se
ofrece cobrarla ni entregarla, y cobrar aparece cuando el pedido está `listo` — antes de
entregar, que es el orden en que ocurre en el mostrador. Un pedido ya `entregado` con
saldo sigue ofreciendo **Cobrar**: así se salda un fiado.

Esto **no contradice** el Plan 0001 D13 ni lo desactiva: entregar desde cualquier estado
vivo sigue siendo válido y sigue usándose, pero por su camino — Caja → Entregas y cobros
(§7.1.1), donde el cliente está enfrente y la cadena no se recorre. Esta pantalla es la del
taller, y ofrecer los cuatro botones a la vez es cómo se entrega un pedido que todavía está
lavándose.

**Agregar servicios** (`/orders/:id/services`) abre la lista del catálogo tal como la dibuja
la toma de pedido (§5.2), con lo ya cobrado marcado: es el secado que se decide cuando la
ropa ya entró. Guarda como corrección del pedido (Plan 0001 §7.3), con el mismo permiso que
**Editar** — `orders.update`, y `orders.update_ready` si el pedido está `listo`.
...
Entregar **no exige** que el pedido esté `ready` (Plan 0001 D13): el backend lo acepta desde
`received`, y Caja lo usa así. Lo que sí espera a `listo` es el botón del **detalle** (§5.4),
por la cadena del taller — no por una restricción del servidor.
...
| Tocar la fila | Abre la **hoja de esa boleta** y la entrega de una vez. Es el camino ordinario: el cliente está enfrente | Una boleta, un cobro, un acto |
| Casilla | Marca para el repaso en lote. Blanco propio, separado del de la fila: tocar una boleta y que se marque en silencio es cómo alguien se va creyendo que ya la entregó | |
...
| Hoja de una boleta | Total, anticipos, **saldo**; ¿cuánto pagó? (**Pagó todo** / **Queda debiendo** con monto); **¿cómo pagó?** (Efectivo / Transferencia + referencia) | Idéntica venga del escáner, del buscador o de la lista |
...
reporta como advertencia (§7.4) sin anularlas. Lo que **sí** sale de la lista es lo entregado:
una boleta que se cerró —desde aquí, desde su detalle o desde otro teléfono— deja de
aparecer y deja de contar entre las marcadas.
...
| Salir | **X** en la AppBar | Una venta se abandona, no se "vuelve" de ella: es la misma salida que la hoja de un gasto. Sobre AppBar blanca hay que fijar `foregroundColor` a mano —el tema pinta los iconos en blanco para el magenta— o el botón queda invisible y solo queda el back del teléfono |
...
| `AppBottomSheetScaffold` (título, contenido scroll, acciones) | Molécula | Todos los formularios en sheet. **El scroll lo hace la scaffold**: el contenido no debe traer el suyo, y si tiene que ser una lista va con `physics: NeverScrollableScrollPhysics()` |
...
| 2026-08-09 | **Nueve correcciones pedidas por el cliente sobre la fase 1 en uso.** Las de Caja: **(1)** tocar una boleta de «Entregas y cobros» abre **su hoja** —¿cuánto pagó (todo / queda debiendo) y cómo pagó (efectivo / transferencia)?— y la entrega de una vez; el §7.1.1 ya la especificaba y solo estaba construida la hoja de lote, que era el camino largo para el caso normal: un cliente enfrente pidiendo su boleta. La casilla se queda para el repaso del cierre, con blanco propio, porque tocar una boleta y que se marque en silencio es cómo alguien se va creyendo que ya la entregó. **(2)** Lo entregado **sale de lo marcado**: el lote se arma solo con boletas todavía abiertas y se desmarca lo que sí salió aunque el lote falle a medias, y para eso `deliverAll` devuelve los **ids** entregados y no un conteo. Las del detalle (§5.4): **(3)** la pantalla ofrece **un paso a la vez** —proceso, listo, y solo entonces cobrar y entregar—, que es la cadena del taller; entregar desde cualquier estado sigue vivo donde el cliente está enfrente (Caja, plan 0001 D13), y un pedido ya entregado con saldo sigue ofreciendo cobrar porque así se salda un fiado. **(4) Agregar servicios** (`/orders/:id/services`) abre la lista del catálogo tal como la dibuja la toma de pedido, con lo ya cobrado marcado: es el secado que se decide cuando la ropa ya entró, y hasta hoy obligaba a reabrir la boleta entera por «Editar» —siete secciones para tocar una—. Reusa el controlador de la captura en modo corrección, así que guardar no pierde prendas ni descuentos. Las de la toma (§5.2): **(5)** el **NIT sale del cliente**, el suyo o `CF` si no tiene, y cambiar de cliente trae el de ese cliente en vez de dejar el anterior —un NIT heredado es una factura a nombre de quien no vino—; **(6) «Limpiar boleta» pregunta antes**, porque vive donde cae el pulgar al terminar de contar prendas y lo que borra no está guardado en ninguna parte; **(7)** el **comprobante** del §17.1 lleva cliente, teléfono, NIT, las prendas contadas y la **hora de recepción**, que es lo que se coteja al recoger la ropa —con solo el total no se puede discutir nada—; **(8)** en la confirmación, **«Listo»** toma el magenta y el sitio del pulgar y «Nuevo pedido» queda sin fondo: tomar otra boleta seguida es la excepción, no lo que pasa después de cada pedido. Y **(9)** en el catálogo, `wash_tub` pasa a llamarse **«Lavado»** a secas —el tamaño ya lo dice la opción, y la unidad sigue siendo la tina—; para que el cambio llegue a una base que ya tiene la fila, `seed_catalog` ahora **reconcilia la redacción** (nombre, unidad, rangos) sin tocar precios, y solo asigna lo que difiere para no dar posición de feed a una escritura que no cambia nada; el cambio viaja a los teléfonos por el feed como cualquier otro. La contrapartida queda escrita en el script: el archivo es la fuente de la redacción, así que un cambio de nombre hecho desde la pantalla de catálogo se revierte la próxima vez que se corra. Verificado: 469 pruebas de la app en verde, `flutter analyze` limpio y las 425 unitarias del backend. Sigue en pie la deuda de `ruff check`. |
| 2026-08-09 | **Tres arreglos más de la fase 1 en uso, uno de ellos sistémico.** **(1) El NIT se muda del encabezado a la sección del cliente** en la toma de pedido y solo aparece con un cliente elegido (detalle en el [plan 0002](../0002-ui-toma-pedido/PLAN.md) §3.2): es un dato *de él*, y soltar al cliente ahora se lo lleva. **(2) Las bottom sheets no rodaban.** `AppBottomSheetScaffold` ya envuelve su contenido en un `SingleChildScrollView`, pero once formularios metían adentro un `ListView(shrinkWrap: true)`: el scrollable de adentro gana el gesto de arrastre y, viniendo del alto exacto de su contenido, no tiene nada que desplazar — así que la sheet se quedaba quieta y lo que caía bajo el pliegue era **inalcanzable**. Lo reportó el cliente donde más dolía, el **TOTAL DE LA COMPRA** del alta de lote (§8.4): no se podía corregir el gasto de una compra. Arreglado en los once con `physics: NeverScrollableScrollPhysics()` —el scroll lo hace la scaffold, ahora escrito en §15 y en el docstring del componente— y con un test que monta la sheet **modal y en 360×640**, que es lo que el test viejo no hacía: a 1600 de alto no hay nada que desplazar y por eso el defecto vivió tanto. **(3) La venta de insumo estrena X para salir** (§7.3). No era falta de botón sino de contraste: el tema pinta los iconos de la AppBar en blanco porque casi todas van sobre el magenta, y estas tres van sobre blanco. La flecha estaba ahí, invisible, y solo quedaba el back del teléfono. En la venta va una X —una venta se abandona, no se vuelve de ella—; entrega y «Agregar servicios» conservan la flecha, ya visible. |
| 2026-08-10 | **Cinco arreglos de la fase 1 en uso, y una regla de negocio que cambia.** **(1) El gasto no se podía anotar**: la hoja del §7.2 recibía las categorías con un `ref.read` en el instante de abrirse, y `cashExpensesCategories` es un stream *autoDispose* que nadie más observaba —la primera lectura devuelve «cargando», no la lista—. Llegaban siempre vacías: la sheet decía que el teléfono no las había bajado y «Registrar» no hacía nada, con las seis categorías en la base. Ahora la sheet **observa** el provider, distingue «cargando» de «no hay ninguna», y cuando de verdad no hay ninguna apaga el botón y ofrece sincronizar ahí mismo en vez de dejar un botón mudo. **(2) Los insumos se cuentan en el gasto**, como las prendas de una boleta: un bloque plegable con los productos del espejo local, su precio de venta y un stepper; tres detergentes llenan el concepto («3 Detergente») y el monto (3 × precio), y mueven la categoría a «Compra de insumos». Los dos campos siguen siendo editables y una vez tocados no se pisan (D8), y un producto sin precio suma al concepto pero no al monto: inventarlo sería poner una cifra que nadie pagó. **No mueve inventario** —el stock lo mueve el alta de lote (§8.4)—, y así lo dice la sheet. **(3) El anticipo puede pasarse del total.** Era un bloqueo en la app y un `ConflictError` en el backend, en el alta y en la corrección; y es exactamente lo que hace el mostrador: cuando la ropa entra nadie sabe qué tratamientos va a necesitar, así que el cliente deja Q100 sobre una boleta que cierra en Q80 y los Q20 se devuelven al entregar. Ahora el saldo puede ser negativo y toda la cadena lo trata como **saldo a favor**: la toma lo dice al teclearlo, la fila de Caja pinta «a favor» en vez de un saldo negativo, la hoja de entrega cambia «¿cuánto pagó?» por «hay que devolverle Q20» y confirma sin registrar un cobro de Q0 —antes moría con «el monto no puede ser negativo» y la ropa no se podía entregar—, el detalle y el comprobante lo imprimen, y entregar una boleta pagada de más **no exige** `orders.deliver_unpaid`, porque no se está fiando nada. Lo que sigue bloqueado es cobrar sobre una boleta sin saldo: eso sí sería vuelto anotado como ingreso. **Queda una deuda contable que este cambio no resuelve**: la devolución sale del cajón y ningún movimiento la registra, así que el arqueo del día en que se entrega queda corto por esa cantidad. **(4) Las filas de Caja llevan los dos números** —el correlativo `#7` y la serie de imprenta— en las entregas pendientes, en el repaso en lote y en los ingresos del día: la fila enseñaba solo la serie, y el papel que alguien tiene en la mano y la app se llamaban distinto. **(5) La administración de insumos ya no exige ir a Sincronizar y volver**: crear un producto, registrar un lote o anotar un movimiento escriben en línea, pero estas pantallas leen del espejo local, así que lo recién creado no existía para ellas hasta el siguiente pull. Ahora la escritura espera un ciclo antes de darse por hecha. Y el *placeholder* del NIT pierde el guion (`12345678`), que se copiaba tal cual y dejaba el mismo NIT escrito de dos maneras. Verificado: 489 pruebas de backend y 497 de la app, `flutter analyze` limpio, `ruff` limpio y `mypy` sin errores nuevos. |
