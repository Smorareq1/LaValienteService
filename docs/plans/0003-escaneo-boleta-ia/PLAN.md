...
| 2026-08-09 | **Segunda pregunta a la misma lectura: `POST /scans/lookup`** (Plan 0006 §7.1.1). El módulo solo sabía leer una boleta para *crear* un pedido; el mostrador también necesita señalar una que ya existe para entregarla. Es el mismo extractor, el mismo proveedor y el **mismo prompt** —un prompt magro para leer solo el encabezado sería una segunda versión que mantener, que sincronizar con el papel cuando la imprenta cambie el formulario y que regresar aparte, a cambio de unos centavos por llamada—; lo que cambia es todo lo de después: se resuelven el serial impreso y el «No.» manuscrito contra `orders` y se devuelve la boleta, sin precios, sin matching y sin borrador. Tres consecuencias en el módulo. **(a)** `scan_jobs.purpose` (`intake`/`lookup`, migración `20260809_0014`) separa las dos: comparten el cupo diario de §8 —el presupuesto es de la lavandería— pero **no** la métrica de exactitud de §9, que solo tiene sentido sobre borradores que alguien corrigió. **(b)** La búsqueda **no guarda la foto**: el §4 la conserva para la revisión lado a lado, y acá no hay nada que revisar porque la persona tiene el papel en la mano. `image_path` vacío, que es lo que `list_expired` ya lee como «no hay archivo». **(c)** `normalize_serial` entra al §7 y se aplica también a la captura: el papel imprime `#Tomapedido`, así que el modelo lee el `#`, y sin normalizar `A-0042` y `#a-0042` serían dos boletas para el índice de duplicados. La dirección de D2 se mantiene: el módulo pasó a leer `orders`, nunca a escribirlos. |
