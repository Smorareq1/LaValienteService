"""create normalized identity and authorization schema
...
def upgrade() -> None:
    # create_type=False: los tipos se crean aquí una sola vez; las columnas no
    # deben volver a emitir CREATE TYPE al crear sus tablas.
    permission_effect = postgresql.ENUM(
        "grant", "deny", name="permission_effect", create_type=False
    )
    permission_effect.create(op.get_bind(), checkfirst=True)
    reset_channel = postgresql.ENUM("email", "sms", name="reset_channel", create_type=False)
    reset_channel.create(op.get_bind(), checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column("channel", reset_channel, nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_table("users")
    sa.Enum(name="reset_channel").drop(op.get_bind(), checkfirst=True)
    sa.Enum(name="permission_effect").drop(op.get_bind(), checkfirst=True)
